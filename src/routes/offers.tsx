...
import { Search, Sparkles, Download, ArrowRight, Eye, RotateCcw } from "lucide-react";
...
function OfferBuilder() {
  ...
  const [editedCaptionTa, setEditedCaptionTa] = useState("");
  ...
  const calcNew  = (mrp: number) =>
    discountType === "percent"
      ? Math.round(mrp - (mrp * discountValue) / 100)
  ...
  const buildCaptionTa = () => {
    if (selectedProducts.length === 0) return "ஒரு பொருளைத் தேர்ந்தெடுக்கவும்.";
    const lines = selectedProducts
      .map((p) => `• ${p.name} – ₹${p.mrp} இருந்து ₹${calcNew(p.mrp)} (₹${savePer(p.mrp)} சேமிப்பு)`)
      .join("\n");
    return `🛒 ஆதிரை மார்ட்டில் சிறப்பு தள்ளுபடி!\n\n${lines}\n\n${validFromLabel} – ${validToLabel} வரை மட்டுமே 🔥\n📞 98765 43200`;
  };

  // Auto-regenerate captions when selection / discount / dates change
  useEffect(() => {
    setEditedCaptionEn(buildCaptionEn());
    setEditedCaptionTa(buildCaptionTa());
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected, discountValue, discountType, validFrom, validTo]);
  ...
  const approve = () => {
    offerStore.set({
      ...
      captionEn:    editedCaptionEn,
      captionTa:    editedCaptionTa,
    });
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
                ...
                <label key={p.id} className="flex items-center gap-3 px-3 py-2.5 hover:bg-muted/50 cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {p.stock} {p.unit} · ₹{p.mrp}
                      {p.expiryDays !== undefined && p.expiryDays <= 7
                        ? ` · ⚠️ ${p.expiryDays}d exp`
                        : ""}
                    </div>
                  </div>
                  {p.aiPick && (
          ...
          <div className="bg-card rounded-xl shadow-card p-5 space-y-3">
            ...
            <textarea
              value={lang === "en" ? editedCaptionEn : editedCaptionTa}
              onChange={(e) =>
                lang === "en"
                  ? setEditedCaptionEn(e.target.value)
                  : setEditedCaptionTa(e.target.value)
              }
              rows={9}
              className="w-full text-sm border border-border rounded-lg p-3 bg-background resize-none focus:outline-none focus:ring-2 focus:ring-ring/30 leading-relaxed"
              placeholder="Select products to generate caption..."
            />

            <button
              onClick={resetCaptions}
              className="text-xs text-primary hover:underline flex items-center gap-1 transition-opacity opacity-70 hover:opacity-100"
            >
              ...
              <div className="mt-4 p-4 border border-border rounded-xl bg-accent/20 space-y-4 animate-in fade-in slide-in-from-top-2">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full text-sm border border-border rounded-lg p-3 bg-background resize-none focus:outline-none focus:ring-2 focus:ring-primary/50"
                    rows={2}
...
function PosterRenderer({
  ...
  const configs: Record<PosterVariant, {
    ...
    custom: {
      ...
      priceColor:  "#fff",    badgeBg:   customColor, badgeText: "#fff",
      bannerBg:    "rgba(0,0,0,0.4)", footerOp: 0.9,
    },
  };
  ...
  return (
    ...
    >
      ...
      <div style={{
        position: "absolute",
        top: variant === "festive" ? 44 : 32, right: 32,
      ...
      <div style={{
        textAlign: "center",
        fontSize: hero ? 56 : 44,
        fontWeight: 900, color: c.titleColor,
        letterSpacing: 1, textShadow: "0 3px 0 rgba(0,0,0,0.15)",
      }}>
        ...
        {items.map((p) => {
          ...
          return (
            <div key={p.id} style={{
              ...
              border: `1px solid ${variant === "modern" ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.15)"}`,
              boxShadow: "0 8px 32px rgba(0,0,0,0.12)",
            }}>
              <div style={{ fontSize: hero ? 36 : 20, fontWeight: 700, color: c.headerColor, lineHeight: 1.2, marginBottom: 10 }}>
                {p.name}
              </div>
              ...
              <div style={{
                ...
                background: c.badgeBg, color: c.badgeText,
                fontWeight: 800,
                padding: hero ? "8px 20px" : "5px 14px",
                borderRadius: 999,
                fontSize: hero ? 18 : 13,
              }}>
      ...
      <div style={{
        marginTop: 28,
        background: c.bannerBg, borderRadius: 16,
...
function ResponsiveScaler({ children, width, height }: { children: React.ReactNode; width: number; height: number }) {
  return (
    <div
      className="w-full relative rounded-xl overflow-hidden bg-muted flex items-center justify-center"
